<?php
	add_shortcode( 'team-up', function( $atts ){
		...
		if( $group_query->have_posts() ){ ?>
			<?php do_action( Team_Up::team_up_archive_default_filter() ); ?>
			<br><br>
			<div class="team-up-container team-up team-up-grid team-up-shortcode team-up-shortcode-group team-up-<?php echo $size; ?>" <?php echo ( $align ) ? "style='text-align: $align;'" : ''; ?>>
				<?php while( $group_query->have_posts() ){ $group_query->the_post();
					...
					<?php if( $break_after == true && filter_var( get_post_meta( $team_id, '_team_up_break_after', true ), FILTER_VALIDATE_BOOLEAN ) ) echo '<div style="clear: both;"></div>'; ?>
